...
/**
 * Checks to see if a value is an Image URL. This means (case-insensitive) a
 * String value that has no spaces and is either a PNG, JPG, JPEG, GIF, SVG or TIFF file.
 * Also permits base64 encoded strings, formatted for the web.
 *
 * @func
 * @sig * -> Boolean
 * @param {String} url A value that needs to be an Image URL
 * @returns {Boolean} whether or not the value is an Image URL
 */
export const isImageUrl = allPass([
    is(String),
    test(/^\S*$/),
    either(
        test(/^(data:image\/)(gif|svg|jpg|jpeg|png|tiff)(;base64,)(\S)+$/i),
        test(/\.(gif|svg|jpg|jpeg|png|tiff)$/i)
    )
])
