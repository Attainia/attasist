 ...
 * Could be a string message or an object with a "message", "detail", "error",
 * "error_description", or "statusText" property at the top-level of the object
 * or nested inside of a "response", "data" or "response.data" object
 * (many JS client libraries tuck their payloads inside of a "data" object).
 *
 * @func
...
export const parseError = pipe(
    ...
    cond([
        ...
        [prop('error_description'), prop('error_description')],
        [path(['data', 'error_description']), path(['data', 'error_description'])],
        [path(['response', 'data', 'error_description']), path(['response', 'data', 'error_description'])],
        [path(['response', 'error_description']), path(['response', 'error_description'])],
...
/**
 * Builds a URL from a base, endpoint, and an object of params (for the query string).
 *
 * @func
 * @sig String -> String -> Object -> String
 * @returns {undefined}
 */
export const makeApiUrl = (baseUrl, endpoint, params = {}) => {
    const base = when(test(/\/$/), init)(baseUrl || '')
    const end = when(test(/^\//), tail)(endpoint || '')
    const url = compose(join('/'), filter(isStringieThingie))([base, end])
    return `${url}${safeQs(params)}`
}
