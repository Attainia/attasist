...
import qs from 'qs'
import {isStringieThingie, isPlainObj} from './validations'

/**
 * Turns a value into an Array (unless it's already an Array)
 * 
 * @func
 * @sig * -> Array
 * @param {*} val A value to coerce to Array
 * @returns {Array} Wraps the value in an Array (unless the value _is_ an Array already)
 */
export const listMe = unless(is(Array), of)

/**
 * Merges two lists of objects together, making sure the final list is unique on
 * a specified prop AND has no blank entries at the second specified prop.
 *
 * @func
 * @sig (String, String) -> Array -> Array -> Array
 * @returns {Array} A list of objects, where the list is unique by the specified
 * prop (optional) and any entries that are blank/null/undefined at the
 * speficied display prop (optional)
 */
export const mergeObjectList = (displayProp, uniqProp) => {
    const makeListUnique = isStringieThingie(uniqProp) ? uniqBy(prop(uniqProp)) : identity
    const pruneBlanks = isStringieThingie(displayProp) ? filter(path(listMe(displayProp))) : identity
    return curry(
        (listA, listB) => compose(
            pruneBlanks,
            makeListUnique,
            reject(anyPass([isNil, isEmpty, complement(isPlainObj)]))
        )(concat(listMe(listA), listMe(listB)))
    )
}

/**
 * Replaces null values with undefined. Useful when you want the default value
 * of native JavaScript to take over, but wouldn't if the value was actually
 * "null"   
 *
 * @func
 * @sig * -> *|undefined
 * @param {*} val A value that may be 
 * @returns {*} The original value, unless that was null, in which case
 * undefined will be returned (duh)
 */
export const undefineMe = when(equals(null), always(undefined))
 ...
 */
export const etc = curry(
    (endIndex, str) => {
        const whole = `${isStringieThingie(str) ? str : ''}`
        const end = is(Number, endIndex) ? endIndex : 100
        const sliced = slice(0, end, whole)
        return whole.length > sliced.length ? `${sliced}...` : whole
    }
)
